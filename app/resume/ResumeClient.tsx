...
export default function ResumeClient() {
  ...
  return (
    <div className="flex min-h-[60vh] flex-col items-center justify-center">
      ...
      <a
        className="text-muted-foreground hover:text-foreground mt-4 text-center text-xs underline underline-offset-2 transition-colors"
...
function renderInline(text: string, keyPrefix: string) {
  ...
  return tokens.map((token, index) => {
    ...
    if (token.startsWith('**') && token.endsWith('**')) {
      return (
        <strong className="text-foreground font-medium" key={key}>
          {token.slice(2, -2)}
        </strong>
      );
    }
    ...
    if (token.startsWith('`') && token.endsWith('`')) {
      return (
        <code className="bg-muted rounded px-1 py-0.5 text-[12px]" key={key}>
          {token.slice(1, -1)}
        </code>
      );
    }
    ...
    if (linkMatch) {
      ...
      return (
        <Link
          className="text-foreground hover:text-foreground/60 underline underline-offset-2 transition-colors"
          href={href}
          key={key}
      ...
      <div className="space-y-0.5 font-mono text-[13px] leading-relaxed">
        {lines.map((line, index) => {
          ...
          if (line.startsWith('## ')) {
            return (
              <h2
                className="text-muted-foreground mt-6 mb-1 text-[13px] font-medium tracking-wide uppercase"
                key={key}
              >
          ...
          if (line.startsWith('### ')) {
            return (
              <h3
                className="text-foreground mt-3 text-[13px] font-medium"
                key={key}
              >
